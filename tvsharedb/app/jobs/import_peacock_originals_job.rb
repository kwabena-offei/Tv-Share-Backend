class ImportPeacockOriginalsJob < ApplicationJob
  ...
  # Broader section headers sometimes grouping categories
  ...
  def import_from_wikipedia_doc(doc, seen_ids)
    ...
    # 2) If nothing imported, scan broader headers for lists/tables
  ...
  def import_show(program)
    show = Show.find_or_initialize_by({
      original_streaming_network: :peacock,
