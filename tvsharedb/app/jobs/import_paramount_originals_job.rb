class ImportParamountOriginalsJob < ApplicationJob
  ...
  # Categories commonly used on the Paramount+ Wikipedia page
  ...
  # Broader section headers that sometimes encompass lists/tables
  ...
  def import_from_wikipedia_doc(doc, seen_ids)
    ...
    # 1) Scan allowed category sections first
    ALLOWED_CATEGORIES.each do |category|
      header = doc.at(".mw-headline:contains('#{category}')")
      next unless header

      section = header.parent
      # Paramount+ page typically uses wikitables; first cell has the title
    ...
    # 2) If nothing imported via categories, scan broader headers for lists/tables
  ...
  def import_show(program)
    show = Show.find_or_initialize_by({
      original_streaming_network: :paramount,
