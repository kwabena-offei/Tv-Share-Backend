class ImportShowJob < ApplicationJob
  ...
  def perform(options)
    ...
    if show.is_show?
      consider_importing_episodes(program: program, show: show)
      show.save # update episode count on parent show
    end
  end

  def consider_importing_episodes(program:, show:)
    external_episode_count = program['totalEpisodes']&.to_i
    internal_episode_count = show.episodes_count&.to_i
    return if external_episode_count == internal_episode_count
