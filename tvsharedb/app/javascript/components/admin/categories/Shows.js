...
import {
  Empty, Divider, Input, Switch, Button, message,
} from 'antd';
...
const QuoteItem = () => <div style={{ background: '#fff', margin: 20, minHeight: 500 }} />;
...
function QuoteApp({ category, onDelete }) {
    ...
    return (
      <Draggable draggableId={quote.tmsId} index={index} style={{ userSelect: 'none' }}>
        {(provided) => (
          <div
            style={{
              display: 'flex', overflow: 'auto', flexDirection: 'row', alignItems: 'flex-start',
            }}
          ...
          >
            <div style={{
              background: '#fff', padding: 20, marginRight: 20, minHeight: 480, width: 280,
            }}
            >
  ...
  const QuoteList = React.memo(({ quotes }) => quotes.map((quote, index) => (React.createElement(Quote, { quote, index, key: quote.tmsId }))));
    ...
    <div>
      ...
      <div>
        <div>
          ...
          <div style={{
            display: 'flex', justifyContent: 'space-between', width: 500, margin: '0 auto',
          }}
          >
            ...
            <label>
              Active
              <Switch checked={draftActive} onChange={(checked) => setDraftActive(checked)} />
            </label>
