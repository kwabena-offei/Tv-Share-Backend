require 'test_helper'

class CommentsControllerTest < ActionDispatch::IntegrationTest
  setup do
    @comment = comments(:one)
    @user = @comment.user
  end

  test "should get index" do
    get comments_url, as: :json, headers: auth_header(@user)
    assert_response :success
  end

  test "should create comment" do
    assert_difference('Comment.count') do
      post comments_url, params: { comment: { hashtag: @comment.hashtag, text: @comment.text, user_id: @comment.user_id, show_id: @comment.show_id } }, as: :json, headers: auth_header(@user)
    end

    assert_response 200
  end

  test "should show comment" do
    get comment_url(@comment), as: :json, headers: auth_header(@user)
    assert_response :success
  end

  test "should update comment" do
    patch comment_url(@comment), params: { comment: { hashtag: @comment.hashtag, text: @comment.text, user_id: @comment.user_id } }, as: :json, headers: auth_header(@user)
    assert_response 200
  end

  test "should destroy comment" do
    assert_difference('Comment.count', -1) do
      delete comment_url(@comment), as: :json, headers: auth_header(@user)
    end

    assert_response 204
  end
end
